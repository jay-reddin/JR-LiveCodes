// src/livecodes/editor/monaco/languages/monaco-lang-imba.ts
function iter$__(a) {
  let v;
...
var states = {
  root: [
    ...
    // want to push this state _ before the token
    [
      /^(\t+)(?=[^\t\n])/,
      {
        cases: {
...
function rewrite\u039Estate(raw) {
  ...
  for (let i\u03C62 = 0, items\u03C6 = iter$__(raw.split(/(?=[\/\&\=\*])/)), len\u03C6 = items\u03C6.length; i\u03C62 < len\u03C6; i\u03C62++) {
    let part = items\u03C6[i\u03C62];
    if (part[0] == "&") {
      if (part[1] == "-" || part[1] == "_") {
...
export {
  monaco_lang_imba_default as default
};
