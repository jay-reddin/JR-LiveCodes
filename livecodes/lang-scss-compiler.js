...
(() => {
  ...
  self.createScssCompiler = () => {
    ...
    return async (code, { config, language }) => {
      ...
      const fetchStyles = (url) => {
        const moduleUrl = baseUrl && baseUrl !== url ? new URL(url.replace(moduleServiceUrl, ""), baseUrl).href : url;
        const module = moduleUrl.replace(moduleServiceUrl, "");
        const moduleParts = module.split("/");
        if (moduleParts.length === 1 || moduleParts.length === 2 && moduleParts[0].startsWith("@")) {
          return Promise.reject("not a valid URL!");
        }
        return fetch(moduleUrl).then((res) => {
          if (!res.ok)
            return Promise.reject("URL not found: " + moduleUrl);
          return res.text();
        }).then((contents) => {
          const result2 = {
            contents,
        ...
        importers: [
          {
            ...
            load(canonicalUrl) {
              ...
              return fetchStyles(urlString).catch(() => {
              ...
              }).catch(() => fetchStyles(urlString + ".scss")).catch(
                () => new Promise((resolve, reject) => {
                  fetch(urlString + "/package.json").then((res) => res.json()).then((pkg) => {
                    if (pkg.sass) {
