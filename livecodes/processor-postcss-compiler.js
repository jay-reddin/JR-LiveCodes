...
(() => {
  ...
  var isRelativeUrl = (url) => !url?.startsWith("http") && !url?.startsWith("data:");
  var getAbsoluteUrl = (url, baseUrl = document.baseURI) => isRelativeUrl(url) ? new URL(url, baseUrl).href : url;
...
${modified}
  ...
  // src/livecodes/languages/postcss/processor-postcss-compiler.ts
  var getSpecs = (pluginName) => window.deps.processors.find((specs) => specs.name === pluginName);
  self.createPostcssCompiler = () => {
    const postCssOptions = { from: void 0 };
    const loadedPlugins = {};
    const loadPlugin = (pluginName, baseUrl) => {
      const specs = getSpecs(pluginName);
      if (!specs || loadedPlugins[pluginName] != null)
        return;
      try {
        if (specs.compiler.url) {
          self.importScripts(getAbsoluteUrl(specs.compiler.url, baseUrl));
        }
    ...
    const getEnabledPluginNames = (config) => {
      const configPlugins = config.processors.filter((p) => getSpecs(p)?.isPostcssPlugin);
      const isEnabled = (pluginName) => processorIsEnabled(pluginName, config) && configPlugins.includes(pluginName);
      return window.deps.processors.map((plugin) => plugin.name).filter(isEnabled);
    };
    const getPlugins = (config, baseUrl, options) => {
      const pluginNames = getEnabledPluginNames(config);
      pluginNames.forEach((pluginName) => loadPlugin(pluginName, baseUrl));
      return window.deps.processors.filter((specs) => pluginNames.includes(specs.name)).map((specs) => loadedPlugins[specs.name]?.(config, baseUrl, options)).flat();
    };
    const prepareCode = (code) => escapeCode(
      replaceStyleImports(code),
      /* slash = */
      false
    );
    return async function process2(code, { config, baseUrl, options }) {
      if (!config || !baseUrl)
        return { code, info: {} };
      ...
      const result = (await self.postcss.postcss(plugins).process(prepareCode(code), postCssOptions)).css;
