...
var pgliteUrl = /* @__PURE__ */ getUrl("@electric-sql/pglite@0.1.5/dist/index.js");

// src/livecodes/languages/postgresql/lang-postgresql-compiler-esm.ts
var pgSqlCompiler = async (code, { config }) => {
  if (!code.trim())
    return "{ data: [] }";
...
var transformData = (data) => data.filter((item) => item.fields.length > 0).map((item) => ({ columns: item.fields.map((field) => field.name), values: item.rows }));
export {
  pgSqlCompiler
};
