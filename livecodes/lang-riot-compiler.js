...
export { ${tokens.join(", ")} };`;
  ...
  // src/livecodes/languages/riot/lang-riot-compiler.ts
  self.createRiotCompiler = () => async (code, { config }) => {
    if (!code)
      return "";
    ...
    const result = await window.riot.compileFromString(processedCode, options);
