...
(() => {
  ...
  // src/livecodes/languages/art-template/lang-art-template-compiler.ts
  self.createArtTemplateCompiler = () => async (code, { config }) => {
    if (!code)
      return "";
    ...
    if (config.customSettings.template?.prerender !== false) {
      const template = self.template.compile(code, options);
