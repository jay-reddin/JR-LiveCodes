...
var project_info_default = `<div id="info-container" class="modal-container">\r
  <div class="modal-title" data-i18n="project.heading">Project Info</div>\r
  <div id="info-screen-container" class="modal-screen-container">\r
    <div class="modal-screen">\r
      <label for="title-input" data-i18n="project.title">Project Title</label>\r
      <input id="title-input" type="text" spellcheck="false" />\r
      <label for="description-textarea" data-i18n="project.desc">Description</label>\r
      <textarea id="description-textarea"></textarea>\r
      <label for="head-textarea" data-i18n="project.head" data-i18n-prop="innerHTML"\r
        >Add to &lt;head&gt;</label\r
      >\r
      <textarea\r
        dir="ltr"\r
        id="head-textarea"\r
        placeholder='&lt;meta name="viewport" content="width=device-width, initial-scale=1.0" /&gt;'\r
        spellcheck="false"\r
      ></textarea>\r
      <label for="html-attrs-textarea" data-i18n="project.htmlAttr" data-i18n-prop="innerHTML"\r
        >Attributes for &lt;html&gt;</label\r
      >\r
      <textarea\r
        dir="ltr"\r
        id="html-attrs-textarea"\r
        placeholder='lang="en" class="my-class"'\r
        spellcheck="false"\r
      ></textarea>\r
      <label for="tags-input" data-i18n="project.tags">Tags</label>\r
      <input id="tags-input" type="text" spellcheck="false" />\r
    </div>\r
  </div>\r
</div>\r
`;
...
var result_popup_default = `<!doctype html>\r
<html lang="en">\r
  <head>\r
  ...
  <body>\r
    <script>\r
      let url;\r
      const createIframe = (html) => {\r
        if (!url) return;\r
        const prevIframe = document.querySelector('#result');\r
        if (prevIframe) {\r
          prevIframe.remove();\r
        }\r
        const iframe = document.createElement('iframe');\r
        iframe.id = 'result';\r
        iframe.setAttribute(\r
          'sandbox',\r
          'allow-same-origin allow-downloads allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-presentation allow-scripts',\r
        );\r
        iframe.src = url;\r
        iframe.addEventListener(\r
          'load',\r
          () => {\r
            iframe.contentWindow.postMessage({ html }, '*');\r
          },\r
          { once: true },\r
        );\r
...
\r
      addEventListener('message', (ev) => {\r
        if (ev.source != window.opener) return;\r
        if (ev.data.url) {\r
          url = ev.data.url;\r
          window.opener.postMessage({ type: 'ready' }, '*');\r
          return;\r
        }\r
        const result = ev.data.result;\r
        if (url == null || result == null) return;\r
        createIframe(result);\r
      });\r
\r
      window.opener.postMessage({ type: 'loaded' }, '*');\r
    <\/script>\r
  </body>\r
</html>\r
`;
