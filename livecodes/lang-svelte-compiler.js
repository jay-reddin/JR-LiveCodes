...
export { ${tokens.join(", ")} };`;
  ...
  self.createSvelteCompiler = () => {
    ...
    let imports = {};
    let errors = [];
    const compileSvelteSFC = async (code, { config, language, filename }) => {
      ...
      const isSfc = (mod) => mod.toLowerCase().endsWith(".svelte") || mod.toLowerCase().startsWith("data:text/svelte");
      const fullCode = await replaceSFCImports(code, {
        config,
        filename,
        getLanguageByAlias,
        isSfc,
        compileSFC: async (code2, { config: config2, filename: filename2 }) => {
          const compiled = (await compileSvelteSFC(code2, { config: config2, language, filename: filename2 })).code;
          importedContent += `
${filename2}
...
`;
      ...
      let js;
      try {
        const result = window.svelte.compile(processedCode, {
          css: "injected",
          filename,
          ...customSettings
        });
      ...
      return {
        code: language === "svelte-app" ? `<script type="module">${compiledCode}<\/script>` : compiledCode,
