...
(() => {
  ...
  // src/livecodes/languages/dot/lang-dot-compiler.ts
  self.createDotCompiler = () => async (code, { config }) => {
    ...
    if (config.customSettings.template?.prerender !== false) {
      const template = self.doT.template(code, options);
...
const template = doT.template(\`${escapeCode(code)}\`, ${options ? escapeCode(JSON.stringify(options)) : "null"});
