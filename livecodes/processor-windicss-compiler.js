...
(() => {
  ...
  // src/livecodes/languages/windicss/processor-windicss-compiler.ts
  self.createWindicssCompiler = () => async (css, { config, options }) => {
...
<script>${config.script.content}<\/script></template>`;
    ...
    const { Processor, HTMLParser, CSSParser } = self.windicss;
    ...
    if (customSettings.attributify) {
      ...
      const attrs = htmlParser.parseAttrs().reduceRight((acc, curr) => {
      ...
      const htmlClasses = htmlParser.parseClasses().map((i) => i.result).join(" ");
